...
class User {
  ...
  String? acc; // type of account -> [x, x, x] title, job, details
  ...
  String? id; // user id -> to implement BIG DATA
  String? date; // date of issue
  String? exdate; // date of expiration
  String? years; // unobstructed

  String? parish; // parish name
  String? location;  // location name
  String? deca; // deca name
  String? district; // if Cracow -> district name
  String? council; // council rights [0, 0, 0] if, deca, year of expiration
  String? prize; // prizes record [0, 0] times, date of prize
  ...
  String toToken() {
    var data = "CRMNY";
    data += '+$acc+$name+$surname+$bdate+$pin+$id+$date+$exdate+$years+$parish+$deca+$location+$district+$council+$prize';
    return encrypt(data);
  }
  ...
  Future<bool> valid() async {
    var ex = exdate!.split(".")[2] + exdate!.split(".")[1] + exdate!.split(".")[0];
